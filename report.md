...
Так как явным методам соответсвует ступенчатая таблица, то есть их применение не требует решения системы уравнений, легко реализовать универсальную функцию, которая применяет метод по формулам, используя коэффициенты из таблицы.
...
$$\frac{f_{i}}{dt} = f_{i}(\mathbf{u}, t) (1 \leqslant i \leqslant 2)$$ 
Возьмем новый аргумент - длину дуги интегральной кривой:
$$dl = \rho dt, \rho = \left (1 + \sum f_{i}^{2} \right)^{1/2}$$, тогда
$$\frac{f_{i}}{dl} = F_{i}(\mathbf{u}), F_{i}=f_{i}\rho^{-1} (1 \leqslant i \leqslant 2)$$  
Будем выбирать его в зависимости от кривизны интегральной кривой, по формуле:
$$ T = \frac{T_0}{(1 + L^{0.5/\nu}(\chi, \chi)^{0.25/\nu})^{\nu}}$$
...
$\chi = T_{prev}(\mathbf{F - F_{prev})}$, где $T_{prev}$ - предыдущий шаг.  
