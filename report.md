...
При $\lambda = 10$ наклон равен $0.967$: $\quad \quad$ При $\lambda = 100$ наклон равен $\infty$:

![](img/test2expl1L10.png){ width=200px } ![](img/test2expl1L100.png){ width=200px }

#### Метод Рунге-Кутты 4-го порядка

При $\lambda = 10$ наклон равен $4.180$: $\quad \quad$ При $\lambda = 100$ наклон равен $\infty$:

![](img/test2expl4L10.png){ width=200px } ![](img/test2expl4L100.png){ width=200px }

#### Метод Рунге-Кутты-Фельберга 5-го порядка

При $\lambda = 10$ наклон равен $5.145$: $\quad \quad$ При $\lambda = 100$ наклон равен $\infty$:

![](img/test2expl5L10.png){ width=200px } ![](img/test2expl5L100.png){ width=200px }
...
В координатах $x(t), \, y(t)$ то же самое.

![](img/krasota1.png){ width=500px }

Видно, что у данной системы существуют два стационарных состояния: предельный цикл и
устойчивый фокус, проверим это.
Точкой равновесия является точка ($0.7708774143079162$, $-0.6181508803793435$).
Линеарезуем правую часть нашей системы, с помощью разложения по Тейлору до первого
порядка в окрестности данной точки. Получим линейную систему:
$$
\begin{cases}
x^{\prime} = 0.405748012109941*x + y + 0.305396957248 \\
y^{\prime} = -x - 0.6*y + 0.4
\end{cases}
$$
Собственные знчения данной системы равны $-0.0971259939450294 - 0.864359724902905*i$ и
$-0.0971259939450294 + 0.864359724902905*i$. Это значнит, что тип данной точки -- 
устойчивый фокус. Фазовый портрет в окрестности данной точки:

![](img/phase_portrait.png){ width=250px }
...
Определение жесткой задачи:
При данном наборе ни при каких значениях a колебания не затухают и система переодическая.
Если численный метод с конечной областью абсолютной устойчивости, применяемый к системе с любыми начальными условиями, вынужден использовать в определенном интервале интегрирования длину шага, которая чрезмерно мала по отношению к гладкости точного решения в этом интервале, то система называется жесткой в этом интервале.
